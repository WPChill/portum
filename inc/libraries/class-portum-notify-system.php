...
/**
 * Class Portum_Notify_System
 */
class Portum_Notify_System extends Epsilon_Notify_System {
	/**
	...
	public static function plugin_verifier( $slug = '', $get = '', $plugin_name = '', $special = false ) {
		...
		$arr['title'] = sprintf( __( 'Install: %s', 'portum' ), $plugin_name );
		// Translators: %s is the plugin name.
		$arr['description'] = sprintf( __( 'Please install %s in order to create the demo content.', 'portum' ), $plugin_name );

		if ( $arr['installed'] ) {
			// Translators: %s is the plugin name
			$arr['title'] = sprintf( __( 'Activate: %s', 'portum' ), $plugin_name );
			// Translators: %s is the plugin name
			$arr['description'] = sprintf( __( 'Please activate %s in order to create the demo content.', 'portum' ), $plugin_name );
		}
